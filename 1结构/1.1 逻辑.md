...
两个对象a，b**相等**，记作a=b。如果满足以下公理：
 - 1. ==自反公理== 对任意对象x，有x=x。$$\forall x,~x=x$$
 - 2. ==对称公理== 对任意对象x，y，如果x=y，则y=x。$$\forall x \forall y,~x=y \Rightarrow y=x$$
 - 3. ==传递公理== 对任意对象x，y，z，如果x=y，y=z，则x=z。$$\forall x \forall y \forall z,~x=y \land y=z \Rightarrow x=z$$

 - 4. ==替换公理== 对任意对象x，y，如果x=y，P是关于该对象的任意命题，则P(x)=P(y)。$$\forall x \forall y,~ x=y\Rightarrow P(x)=P(y)$$
...
# 4 证明
**证明**是描述命题推理过程的推理序列。每个推理由**前提引入**、**等价演算**、**得出结论**三个过程组成。**演绎法**/**反证法**：
...
命题$A_1,A_2,...,A_n$称作命题$B$成立所需的**条件**。如果命题是以$A\rightarrow B$的形式给出，且可被证明为真，则称$A$是$B$的**充分条件**，$B$是$A$的**必要条件**。
